...
import { getFilterFields, type FilterFieldConfig } from '../config/filters';
import type { FilterValues } from '../types';

interface FiltersPanelProps {
  onSearch: (filters: Partial<FilterValues>) => void;
  onClear: () => void;
}

/** Placeholder text per filter field name */
...
export function FiltersPanel({ onSearch, onClear }: FiltersPanelProps) {
  const { role } = useRole();

  const fields = useMemo(
    () => (role ? getFilterFields(role) : []),
    [role],
  );
  ...
  return (
    ...
    >
      ...
      {/* Buttons at the bottom-right inside the border */}
